...
/**
 * @title OriginVerifier - Advanced Supply-chain Implementation
 ...
 * @notice Production-ready supply-chain contract #7
 */

contract OriginVerifier {
    // Error definitions
    error Unauthorized();
    error InvalidInput();
    ...
    // Modifiers
    modifier onlyOwner() {
        require(msg.sender == owner, "OriginVerifier: not owner");
        _;
    }
    
    modifier onlyAuthorized() {
        require(authorized[msg.sender] || msg.sender == owner, "OriginVerifier: not authorized");
        _;
    }
    
    modifier notPaused() {
        require(!isPaused, "OriginVerifier: contract paused");
        _;
    }
    ...
    function executeAdvancedOperation(
        ...
        require(inputValue > 0, "OriginVerifier: invalid input");
        require(algorithmType <= 10, "OriginVerifier: invalid algorithm");
    ...
    function updateAdvancedState(
        ...
        require(values.length >= 5, "OriginVerifier: insufficient values");
